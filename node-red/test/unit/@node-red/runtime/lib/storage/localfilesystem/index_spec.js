...
describe('storage/localfilesystem', function() {
    ...
    it('should set userDir to HOMEPATH/.grape-rpa if .config.json presents',function(done) {
        var oldNRH = process.env.NODE_RED_HOME;
        process.env.NODE_RED_HOME = path.join(userDir,"NRH");
        var oldHOMEPATH = process.env.HOMEPATH;
        process.env.HOMEPATH = path.join(userDir,"HOMEPATH");
        fs.mkdirSync(process.env.HOMEPATH);
        fs.mkdirSync(path.join(process.env.HOMEPATH,".grape-rpa"));
        fs.writeFileSync(path.join(process.env.HOMEPATH,".grape-rpa",".config.json"),"{}","utf8");
        var settings = {getUserSettings: () => {{}}};
        localfilesystem.init(settings, mockRuntime).then(function() {
            try {
                fs.existsSync(path.join(process.env.HOMEPATH,".grape-rpa","lib")).should.be.true();
                fs.existsSync(path.join(process.env.HOMEPATH,".grape-rpa","lib",'flows')).should.be.true();
                settings.userDir.should.equal(path.join(process.env.HOMEPATH,".grape-rpa"));
                done();
            } catch(err) {
                done(err);
            } finally {
    ...
    it('should set userDir to HOME/.grape-rpa',function(done) {
        var oldNRH = process.env.NODE_RED_HOME;
        process.env.NODE_RED_HOME = path.join(userDir,"NRH");
        ...
        localfilesystem.init(settings, mockRuntime).then(function() {
            try {
                fs.existsSync(path.join(process.env.HOME,".grape-rpa","lib")).should.be.true();
                fs.existsSync(path.join(process.env.HOME,".grape-rpa","lib",'flows')).should.be.true();
                settings.userDir.should.equal(path.join(process.env.HOME,".grape-rpa"));
                done();
            } catch(err) {
                done(err);
            } finally {
    ...
    it('should set userDir to USERPROFILE/.grape-rpa',function(done) {
        var oldNRH = process.env.NODE_RED_HOME;
        process.env.NODE_RED_HOME = path.join(userDir,"NRH");
        var oldHOME = process.env.HOME;
        process.env.HOME = "";
        ...
        localfilesystem.init(settings, mockRuntime).then(function() {
            try {
                fs.existsSync(path.join(process.env.USERPROFILE,".grape-rpa","lib")).should.be.true();
                fs.existsSync(path.join(process.env.USERPROFILE,".grape-rpa","lib",'flows')).should.be.true();
                settings.userDir.should.equal(path.join(process.env.USERPROFILE,".grape-rpa"));
                done();
            } catch(err) {
                done(err);
            } finally {
