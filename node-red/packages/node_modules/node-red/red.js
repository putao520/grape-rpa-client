...
if (parsedArgs.settings) {
    ...
    if (fs.existsSync(path.join(process.env.NODE_RED_HOME,".config.json"))) {
    ...
    } else if (process.env.HOMEPATH && fs.existsSync(path.join(process.env.HOMEPATH,".grape-rpa",".config.json"))) {
        // Consider compatibility for older versions
        settingsFile = path.join(process.env.HOMEPATH,".grape-rpa","settings.js");
    } else {
        ...
        var userDir = parsedArgs.userDir || path.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH,".grape-rpa");
...
httpsPromise.then(function(startupHttps) {
    ...
    app.get('/proxy/:url', function(req, res) {
        ...
        // 创建一个新的代理服务器
        const port = settings.serverPort + 1
        proxyServer.instance(url, port, res)
    })
    app.get('/closeProxy', function(req, res) {
        // 关闭一个新的代理服务器
...
function exitWhenStopped() {
	if (!stopping) {
		...
		RED.stop().then(function() {
			// 插入环境清理
			const pageStore = RED.server['pageStore']
			if( pageStore ){
				pageStore.del().then(()=>{
					process.exit();
				})
...
function setListenCallback(cb, error_cb) {
    listen_error = error_cb;
    listen_cb = cb;
}
