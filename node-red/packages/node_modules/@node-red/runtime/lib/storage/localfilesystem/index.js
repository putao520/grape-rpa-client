...
var localfilesystem = {
    init: async function(_settings, runtime) {
        ...
        if (!settings.userDir) {
            ...
            } else if (process.env.HOMEPATH && checkForConfigFile(fspath.join(process.env.HOMEPATH,".grape-rpa"))) {
                settings.userDir = fspath.join(process.env.HOMEPATH,".grape-rpa");
            } else {
                settings.userDir = fspath.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH || process.env.NODE_RED_HOME,".grape-rpa");
            }
        }
