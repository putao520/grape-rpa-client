...
Node.prototype._complete = function(msg,error) {
    this.__run_change = true
    if( !this.__run_state ){
        _debugCompleteHandler(this, msg)
    }
...
function getEventContext(ctx){
    return ( ctx && ctx.__user_event_props__ ) ? ctx.__user_event_props__ : undefined
}

function _controllerHandler(node, ctx){
    const eventCtx = getEventContext(ctx)
    // 停止事件
    if( eventCtx.needStop() ){
        // 提示停止
        events.emit("runtime-event",{id:"runtime-state",payload:{type:"warning",error: undefined,text:"notification.warnings.stop"},retain:true});
        _debugCompleteHandler(node, ctx)
        return false
    }
    // 暂停事件
    if( eventCtx.needPause() ){
        events.emit(
            "debug-event",
            {
                id:"pause-on",
                payload:{type:"warning", context: ctx, node: node},
                retain:true}
        );
        // 提示暂停
        events.emit("runtime-event",{id:"runtime-state",payload:{type:"warning",error: undefined,text:"notification.warnings.pause"},retain:true});
        return false
    }
    return true
}

function getDebugContext(ctx){
    return ( ctx && ctx.__user_debug_props__ ) ? ctx.__user_debug_props__ : undefined
}

function deleteSystemProperties(ctx){
    const c = {}
    if( ctx ){
        for(var key in ctx){
...
function _debugHandler(node, ctx){
    const debugCtx = getDebugContext(ctx)

    // 增加中断响应处理
    if( debugCtx ){
        // 忽略节点
        if( debugCtx.needIgnoreId(node)){
            return true
        }
        // 中断
        if( debugCtx.needBreak(node) || debugCtx.isSingleStep() ){
            events.emit(
                "debug-event",
                {
                    id:"break-on",
                    payload:{type:"warning", context: deleteSystemProperties(ctx), node: node},
                    retain:true}
            );
            return false
        }
...
function _debugCompleteHandler(node, arg){
    events.emit(
        "debug-event",
        {
            id:"complete-on",
            payload:{type:"warning", context: deleteSystemProperties(arg), node: node},
            retain:true}
    );
}
    ...
    hooks.trigger("onReceive",receiveEvent,(err) => {
        if (err) {
            ...
            if (node._inputCallback) {
                ...
                try {
                    // 控制器处理
                    if( _controllerHandler(node, arg) === false ){
                        return
                    }
                    // 调试模型
                    if(_debugHandler(node, arg) === false ){
                        return
                    }
                    // 获得调试状态
                    const _self = node
                    _self.__run_state = false
                    _self.__run_change = false
                    _self._inputCallback(
                        arg,
...
Node.prototype.__run_state = false
Node.prototype.__run_change = false
/**
