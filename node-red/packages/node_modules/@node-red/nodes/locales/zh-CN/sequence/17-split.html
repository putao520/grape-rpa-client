...
<script type="text/x-red" data-help-name="split">
    ...
    <dl class="message-properties">
        ...
        <dd>组件的行为由<code>msg.payload</code>的类型决定:
            <ul>
    ...
    <dl class="message-properties">
        ...
        <dd>此属性包含有关如何将消息与原始消息分开的信息。如果传递给<b>join</b>组件，则可以将序列重组为单个消息。该属性具有以下属性：
        <ul>
            ...
            <li><code>key</code> - 对于对象，创建此消息的属性的键。可以将组件配置为也将此值复制到另一个消息属性，例如<code>msg.topic</code></li>
            <li><code>len</code> - 使用固定长度值拆分消息时，每段子消息的长度</li>
        </ul>
        </dd>
    </dl>
    <h3>详细</h3>
    <p>在使用<b>join</b>组件将序列重新组合为单个消息之前，推荐使用此组件来轻松地创建跨消息序列，执行通用操作的流。</p>
    <p>它使用<code>msg.parts</code>属性跟踪序列的各个部分。</p>
    <h4>流媒体模式</h4>
    <p>该组件还可以用于重排消息流。例如，发送换行符终止命令的串行设备可能会传递一条消息，并在其末尾带有部分命令。 在“流模式”下，此组件将拆分一条消息并发送每个完整的段。如果末尾有部分片段，则该组件将保留该片段，并将其添加到收到的下一条消息之前。</p>
    <p>在此模式下运行时，该组件将不会设置<code>msg.parts.count</code>属性，因为流中期望的消息数还是未知的。这意味着它不能在自动模式下与<b>join</b>组件一起使用。</p>
</script>
...
<script type="text/x-red" data-help-name="join">
    ...
    <dl>
        <dt>自动模式</dt>
        <dd>与<b>split</b>组件配对时，它将自动将已被拆分的消息进行合并。</dd>
        <dt>手动模式</dt>
    ...
    <dl class="message-properties">
        ...
        <dd>使用自动模式时，所有的消息都应包含此属性。<b>split</b>组件会生成此属性，但也可以手动进行设置。该属性具有以下属性：
        <ul>
        ...
        <dd>如果设置，则组件将以其当前状态发送其输出消息。</dd>
    </dl>
    <h3>详细</h3>

    <h4>自动模式</h4>
    <p>自动模式使用传入消息的<code>parts</code>属性来确定应如何连接序列。这使它可以自动逆转<b>split</b>组件的操作。</p>

    <h4>手动模式</h4>
    <p>设置为以手动模式时，该组件能以各种不同的方法来处理消息：</p>
    <ul>
    ...
    <h4>储存讯息</h4>
    <p>该组件将在内部缓存消息，以便跨序列工作。运行时设置<code>nodeMessageBufferMaxLength</code>可用于设定缓存的消息数。</p>
</script>
