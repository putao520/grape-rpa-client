...
<script type="text/x-red" data-help-name="sort">
    ...
    <p>当配置为对消息属性进行排序时，组件将对指定消息属性所指向的数组数据进行排序。</p>
    ...
    <p>在对消息序列进行排序时，排序组件依赖于接收到的消息来设置<code>msg.parts</code>。拆分组件将生成此属性，但也可以手动创建。它具有以下属性：</p>
    <p>
        <ul>
    ...
    <p><b>注意：</b>在此组件的处理中，消息在内部存储。通过指定要累积的最大消息数，可以防止意外的高内存使用。默认设置是不限制消息数量。
        <ul>
