...
<script type="text/x-red" data-help-name="function">
    ...
    <h4>传送消息</h4>
    <p>要将消息传递到流中的下一个组件，请返回消息或调用<code>node.send(messages)</code>。</p>
    <p>它将返回/send:</p>
    <ul>
      <li>单个消息对象 - 传递给连接到第一个输出的组件</li>
      <li>消息对象数组，传递给连接到相应输出的组件</li>
    </ul>
    ...
    <p>使用catch组件可以进行错误处理。 要由catch组件处理，请将<code>msg</code>作为<code>node.error</code>的第二个参数传递：</p>
    <pre>node.error("Error",msg);</pre>
    <h4>访问组件信息</h4>
    <p>您可以使用以下属性来在代码中引用组件ID和名称：</p>
    <ul>
        <li><code>node.id</code> - 组件的ID</li>
        <li><code>node.name</code> - 组件的名称</li>
    </ul>
    <h4>使用环境变量</h4>
