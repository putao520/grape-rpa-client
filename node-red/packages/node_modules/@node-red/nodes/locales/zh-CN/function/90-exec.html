...
<script type="text/x-red" data-help-name="exec">
    <p>运行系统命令并返回其输出。</p>
    <p>可以将组件配置为等待命令完成，或者在命令生成时发送其输出。</p>
    <p>运行的命令可以在组件中配置，也可以由收到的消息提供。</p>
    ...
    <p>运行的命令是在组件内定义的，带有附加<code>msg.payload</code>的选项和另外一组参数。</p>
    <p>带空格的命令或参数应该用引号引起来：<code>“这是一个参数”</code></p>
    <p>返回的<code>有效荷载</code>通常是<i>字符串</i>类型，除非检测到非UTF8字符，在这种情况下，它会是<i>buffer</i>类型。</p>
    <p>组件处于活动状态时，该组件的状态图标和PID将可见。对此更改可以通过<code>Status</code>组件读取。</p>
    <h4>杀死进程</h4>
    ...
    <p>如果组件有多个进程在运行，则还必须设置<code>msg.pid</code>并设置要杀死的PID的值。</p>
