...
<script type="text/x-red" data-help-name="http request">
    ...
    <dl class="message-properties">
        ...
        <dd>如果未在组件中配置，则此可选属性设置请求的url。</dd>
        <dt class="optional">method <span class="property-type">字符串</span></dt>
        <dd>如果未在组件中配置，则此可选属性设置请求的HTTP方法。必须是<code>GET</code>,<code>PUT</code>,<code>POST</code>,<code>PATCH</code>或<code>DELETE</code>之一。</dd>
    ...
    <dl class="message-properties">
        ...
        <dd>响应的正文。可以将组件配置为以字符串形式返回主体，尝试将其解析为JSON字符串或将其保留为二进制buffer。</dd>
    ...
    <h3>详细</h3>
    <p>在组件内配置后，URL属性可以包含<a href="http://mustache.github.io/mustache.5.html" target="_blank">mustache样式</a>标签。 这些标签允许使用传入消息的值来构造url。例如，如果url设置为<code>example.com/{{{{topic}}}</code>，它将自动插入<code>msg.topic</code>的值。使用{{{...}}}可以防止mustache转义/ &等字符。</p>
    <p>组件可以选择自动将<code>msg.payload</code>编码为GET请求的查询字符串参数，在这种情况下，<code>msg.payload</code>必须是一个对象。</p>
    <p><b>注意：</b>如果使用了代理，则应设置<code>http_proxy=...</code>环境变量并重新启动Node-RED，或使用“代理配置”。如果设置了代理配置，则配置优先于环境变量。</p>
    <h4>使用多个HTTP请求组件</h4>
    <p>为了在一个流程中多次使用该组件，必须要注意<code>msg.headers</code>属性的处理。通常在第一个组件在响应头中设置此属性，而不期望在下一个组件的请求头中使用此属性。如果组件之间的<code>msg.headers</code>属性保持不变，则第二个组件将忽略它。要设置自定义标题，首先应删除<code>msg.headers</code>或将其重置为空对象：<code>{}</code>。</p>
    <h4>Cookie处理</h4>
    <p>传递给组件的<code>cookies</code>属性必须是‘名称/值’键值对的对象。该值可以是设置cookie值的字符串，也可以是具有单个<code>value</code>属性的对象。</p>
    <p>请求返回的所有cookie都将在<code>responseCookies</code>属性下传递回去。</p>
    <h4>内容类型处理</h4>
    <p>如果<code>msg.payload</code>是一个对象，则组件将自动将请求的内容类型设置为<code>application/json</code>并对其进行编码。</p>
    <p>要将请求编码为表单数据，应将<code>msg.headers[“content-type”]</code>设置为<code>application/x-www-form-urlencoded</code>。</p>
    <h4>文件上传</h4>
    <p>要执行文件上传，应将<code>msg.headers["content-type"]</code>设置为<code>multipart/form-data</code>和<code>msg.payload</code>传递给组件的必须是具有以下结构的对象：</p>
    <pre><code>{
    "KEY": {
        "value": FILE_CONTENTS,
        "options": {
