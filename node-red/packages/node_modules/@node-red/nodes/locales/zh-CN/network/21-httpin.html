...
<script type="text/x-red" data-help-name="http in">
    ...
    <dl class="message-properties">
        ...
        <dd>HTTP请求对象。该对象包含有关请求信息的多个属性。
            <ul>
             ...
             <li><code>files</code> - 如果组件启用了文件上传，则为包含了上传的文件的对象。</li>
            </ul>
        </dd>
        <dt>res<span class="property-type">object</span></dt>
        <dd>HTTP响应对象。此属性不应直接使用；<code>HTTP Response</code>组件记录了如何响应请求。该属性必须保留在传递给响应组件的消息上。</dd>
    </dl>
    <h3>详细</h3>
    <p>组件将在配置的路径上监听特定类型的请求。路径可以完全指定，例如<code>/user</code>，或包括可以接受任何值的命名参数，例如<code>/user/:name</code>。 使用命名参数时，可以在<code>msg.req.params</code>下访问其在请求中的实际值。</p>
    ...
    <p><b>注意：</b>该组件不发送对请求的任何响应。该流必须包含HTTP响应组件才能完成请求。</p>
</script>

<script type="text/x-red" data-help-name="http response">
    <p>将响应发送回从HTTP输入组件接收的请求。</p>
    ...
    <h3>详细</h3>
    <p>还可以在组件本身内设置<code>statusCode</code>和<code>headers</code>。如果在组件内设置了属性，则不能被相应的message属性覆盖。</p>
    <h4>Cookie处理</h4>
