...
<p>连接到MQTT代理并订阅来自指定主题的消息。</p>
    ...
    <p>使用该组件您首先需要建立与MQTT代理的连接。通过单击铅笔图标来进行配置。</p>
    <p>如有需要，几个MQTT组件（输入或输出）可以共享相同的代理连接。</p>
</script>
...
<script type="text/x-red" data-help-name="mqtt out">
    ...
    <p>可以在组件中配置所使用的主题，或者如果留为空白，则可以通过<code>msg.topic</code>进行设置。</p>
    <p>同样，可以在组件中配置QoS和保留值，或者如果保留空白，则分别由<code>msg.qos</code>和<code>msg.retain</code>设置。要清除先前存储在代理中的主题，请设置保留标志并向该主题发布空消息。</p>
    <p>该组件需要与要配置的MQTT代理的连接。通过单击铅笔图标进行配置。</p>
    <p>如果需要，几个MQTT组件（输入或输出）可以共享相同的代理连接。</p>
</script>

<script type="text/x-red" data-help-name="mqtt-broker">
    <p>与MQTT代理的连接设置。</p>
    <p>创建与代理的连接设置。可以在<code>MQTT In</code>和<code>MQTT Out</code>组件中重复利用这些设置。</p>
    <p>如果未为该组件设置客户端ID，并且设置了会话初始化，则将生成一个随机客户端ID。设置客户端ID时，请确保它对于连接目标处的代理是唯一的。</p>
    <h4>Birth Message</h4>
    <p>建立连接后发布在以配置主题中的消息。</p>
    <h4>Close Message</h4>
    <p>在连接正常结束之前重新保存或者关闭了组件时，发布在以配置主题中的消息。</p>
    <h4>Will Message</h4>
    <p>当组件意外丢失连接时由代理发布的消息</p>
    <h4>WebSockets</h4>
    <p>可以将组件配置成使用WebSocket连接。使用WebSocket时，请在服务器字段中以完整格式描述连接目标的URI。 例如：</p>
