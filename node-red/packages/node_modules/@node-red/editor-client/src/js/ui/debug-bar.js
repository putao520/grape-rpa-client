RED.debugBar = (function() {
    ...
    function _Command(path, node, label, customMsg) {
        $.ajax({
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                if (jqXHR.status === 404) {
                    RED.notify(RED._("common.notification.error", { message: RED._("common.notification.errors.not-deployed") }), "error");
                } else if (jqXHR.status === 500) {
                    RED.notify(RED._("common.notification.error", { message: RED._("inject.errors.failed") }), "error");
                } else if (jqXHR.status === 0) {
                    RED.notify(RED._("common.notification.error", { message: RED._("common.notification.errors.no-response") }), "error");
                } else {
                    RED.notify(RED._("common.notification.error", { message: RED._("common.notification.errors.unexpected", { status: jqXHR.status, message: textStatus }) }), "error");
                }
            }
        });
    }
    ...
    function checkFlowNodes() {
        ...
        // 判断节点类型是否为启动(inject)节点
        if( frontNode.type !== 'inject' ){
            RED.notify(RED._("debug.error.type-error"), { type: "error", timeout: 5000 });
            return undefined
        }
    ...
    function doStop(){
        _doEvent('stop', RED._("debug.info.stop"))
    }
    function doPause(){
        _doEvent('pause', RED._("debug.info.pause"))
    }
