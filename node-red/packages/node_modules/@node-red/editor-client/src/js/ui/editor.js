...
RED.editor = (function() {
    ...
    function getNodeDefaults(node){
        const v = node._def.defaults || {};
        // v.breakStatebreakState = {value:false};
        return v
    }
    /**
     * Validate a node
    ...
    function validateNode(node) {
        ...
        var hasChanged;
        const nodeDefaults = getNodeDefaults(node);
        if (node.type.indexOf("subflow:")===0) {
            ...
            validationErrors = validateNodeProperties(node, nodeDefaults, node);
    ...
    function validateNodeEditor(node,prefix) {
        const nodeDefaults = getNodeDefaults(node);
        for (var prop in nodeDefaults) {
            if (nodeDefaults.hasOwnProperty(prop)) {
                validateNodeEditorProperty(node,nodeDefaults,prop,prefix);
            }
        }
    ...
    function handleEditSave(editing_node, editState) {
        ...
        if (editing_node._def.oneditsave) {
            var oldValues = {};
            const nodeDefaults = getNodeDefaults(editing_node)
            for (d in nodeDefaults) {
                if (nodeDefaults.hasOwnProperty(d)) {
    ...
    function showEditDialog(node, defaultTab) {
        ...
        var trayOptions = {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function() {
                        if (editing_node._def) {
                            ...
                            const nodeDefaults = getNodeDefaults(editing_node)
                            for (var d in nodeDefaults) {
                                if (nodeDefaults.hasOwnProperty(d)) {
                                    var def = nodeDefaults[d];
                                    if (def.type) {
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if ($("#node-input-node-breakState").prop('checked')) {
                            if (node.breakState !== true) {
                                // editState.changes.breakState = node.breakState;
                                // editState.changed = true;
                                node.breakState = true;
                                RED.debug.addBreak(node.id);
                            }
                        } else {
                            if (node.breakState === true) {
                                // editState.changes.breakState = node.breakState;
                                // editState.changed = true;
                                delete node.breakState;
                                RED.debug.removeBreak(node.id);
                            }
                        }
            ...
            open: function(tray, done) {
                ...
                var trayFooterBreak = $('<div class="red-ui-tray-footer-left"></div>').appendTo(trayFooter)

                $('<input id="node-input-node-breakState" type="checkbox">').prop("checked",!!node.breakState).appendTo(trayFooterBreak).toggleButton({
                    enabledIcon: "fa-circle-thin",
                    disabledIcon: "fa-ban",
                    enabledLabel: RED._("editor:workspace.break"),
                    disabledLabel: RED._("editor:workspace.running"),
                    invertState: true
                })
                ...
                const nodeDefaults = getNodeDefaults(node);
                if (!nodeDefaults || !nodeDefaults.hasOwnProperty('info'))  {
    ...
    function showEditConfigNodeDialog(name,type,id,prefix,editContext) {
        ...
        var trayOptions = {
            ...
            open: function(tray, done) {
                ...
                var nodeEditPanes = [ 'editor-tab-properties' ];
                const nodeDefaults = getNodeDefaults(editing_config_node)
                if (!nodeDefaults || !nodeDefaults.hasOwnProperty('info'))  {
                    nodeEditPanes.push('editor-tab-description');
                }
